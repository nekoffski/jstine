...
class TestMultiClientFunctional(TestCase):
    def test_parallel_writes_to_distinct_keys(self):
        ...
        def worker(index: int, key: bytes) -> None:
            h = ClientHarness(self, add_cleanup=False)
            try:
                start.wait()
        ...
        probe = ClientHarness(self, add_cleanup=False)
        try:
            for index, key in enumerate(keys):
    ...
    def test_concurrent_updates_are_visible_across_clients(self):
        key = b"multi:shared"
    ...
    def test_delete_and_get_across_independent_clients(self):
        key = b"multi:delete"
        writer = ClientHarness(self, add_cleanup=False)
        reader = ClientHarness(self, add_cleanup=False)
        deleter = ClientHarness(self, add_cleanup=False)
...
class TestAsyncMultiClientFunctional(TestCase):
    async def test_parallel_writes_to_distinct_keys(self):
        harnesses = [
            await AsyncClientHarness.create(self, add_cleanup=False)
            for _ in range(6)
        ]
        ...
        try:
            ...
            probe = await AsyncClientHarness.create(self, add_cleanup=False)
            try:
                for index, key in enumerate(keys):
    ...
    async def test_shared_key_is_visible_across_async_clients(self):
        writer_a = await AsyncClientHarness.create(self, add_cleanup=False)
        writer_b = await AsyncClientHarness.create(self, add_cleanup=False)
        reader = await AsyncClientHarness.create(self, add_cleanup=False)
